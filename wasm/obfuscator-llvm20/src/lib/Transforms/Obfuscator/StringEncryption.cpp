...
namespace {
    ...
    // Section naming
    // ============================================================================
    // Mach-O requires "segment,section" specifiers and doesn't accept the
    // ELF-style leading-dot single-component name; using one crashes codegen
    // with "invalid section specifier". Mirrors the per-target branching
    // TargetInfo::getEncryptedStringSectionName() (TargetCompat.h) already
    // does for the single generic "encrypted string" section -- this file
    // needs several distinct sections (key data/table, ciphertext, nonce,
    // chacha key) so it gets its own small helper instead of overloading that
    // one name.
    static std::string strEncSectionName(const Module& M, StringRef Suffix) {
        llvm::obf::TargetInfo TI = llvm::obf::TargetInfo::fromModule(M);
        std::string S = Suffix.str();
        if (TI.IsMachO)
            return "__DATA,__strenc_" + S;
        if (TI.IsCOFF)
            return ".strenc$" + S;
        return ".strenc." + S;
    }

    // Section names set above are matched back against this, e.g. to hide
    // stub-owned globals/skip re-encrypting them. Handles both the ELF/COFF
    // ".strenc..."-prefixed form and the Mach-O "__DATA,__strenc_..." form.
    static bool isStrEncSection(StringRef Sec) {
        return Sec.starts_with(".strenc") || Sec.contains("__strenc_");
    }
    ...
    GlobalVariable* StrEncImpl::createKeyDataGlobal(Module& M,
        ...
        GV->setSection(strEncSectionName(M, "kd"));
    ...
    Function* StrEncImpl::createKeyCodeFn(Module& M, const uint8_t rk[176]) {
        ...
        F->setLinkage(GlobalValue::PrivateLinkage);
        F->addFnAttr(Attribute::NoUnwind);
        F->setSection(strEncSectionName(M, "kt"));
        F->setUnnamedAddr(GlobalValue::UnnamedAddr::Global);
    ...
    bool StrEncImpl::shouldEncrypt(GlobalVariable& GV, int minLength) {
        ...
        // Skip stub-owned globals (strenc's own key/ciphertext/nonce globals)
    ...
    GlobalVariable* StrEncImpl::createCiphertextGlobal(Module& M,
        ...
        GV->setSection(strEncSectionName(M, "ct"));
    ...
    GlobalVariable* StrEncImpl::createNonceGlobal(Module& M,
        ...
        GV->setSection(strEncSectionName(M, "n"));
    ...
    bool StrEncImpl::encryptStrings(Module& M, StrEncCtx& Ctx) {
        ...
        // Decrypt-once-at-load, not decrypt-on-every-call: the AES path
        // previously re-decrypted into a fresh stack alloca at the entry of
        // *every* function using a given string, on *every call* of that
        // function -- fine for a function called once, but for a function
        // called many times in a hot loop (e.g. a per-rule/per-predicate
        // compiler helper invoked repeatedly during a single bind()), this
        // means paying a full AES-CTR decrypt on every single call for a
        // value that never changes. Measured directly: a real bind() call
        // through ~110 strenc'd cold-path files went from ~65ms to ~365ms.
        // Each candidate now gets one persistent (BSS, zero-initialized)
        // buffer, decrypted exactly once by a module constructor
        // (llvm.global_ctors, run automatically by the dynamic loader
        // before any exported symbol can be called) -- every use, in every
        // function, is redirected to that one shared buffer. This does not
        // weaken the tool's actual threat model (defeating `strings`/static
        // disassembly of the shipped binary: the on-disk file still only
        // ever contains ciphertext either way) -- it only changes *when*
        // the one-time in-memory decrypt happens, not whether the disk
        // artifact leaks plaintext.
        struct CtorEntry {
            GlobalVariable* CtGV;
            GlobalVariable* NonceGV;
            GlobalVariable* CacheBuf;
            ArrayType* CtTy;
            ArrayType* NonceTy;
            uint64_t CtBytes;
        };
        std::vector<CtorEntry> CtorWork;
        ...
        for (auto& Cand : Cands) {
            ...
            // Materialise ConstantExpr users so replaceAllUsesWith below
            // sees every use uniformly.
            ...
            // One persistent, mutable, zero-initialized buffer for this
            // string -- decrypted once (see the constructor built below),
            // then used directly everywhere (no per-function stack copy).
            GlobalVariable* CacheBuf = new GlobalVariable(
                M, CtTy, /*isConstant=*/false,
                GlobalValue::PrivateLinkage,
                ConstantAggregateZero::get(CtTy),
                ".strenc.cache." + std::to_string(Cand.Index));
            CacheBuf->setAlignment(Align(16));
            CacheBuf->setUnnamedAddr(GlobalValue::UnnamedAddr::Global);

            // Redirect every use, in every function, straight to the cache
            // buffer -- replaces the old per-function "find Users, patch
            // each one" loop entirely, since there's now exactly one target
            // for all of them.
            GV->replaceAllUsesWith(CacheBuf);

            CtorWork.push_back({CtGV, NonceGV, CacheBuf, CtTy, NonceTy, CtBytes});
        ...
        // Emit one module constructor that decrypts every cached buffer
        // exactly once, at load time, before any other code in this module
        // can run.
        if (!CtorWork.empty()) {
            FunctionType* CtorFT = FunctionType::get(Type::getVoidTy(C), false);
            Function* CtorFn = Function::Create(
                CtorFT, GlobalValue::InternalLinkage, ".strenc.init", M);
            BasicBlock* BB = BasicBlock::Create(C, "entry", CtorFn);
            IRBuilder<> B(BB);
            for (auto& W : CtorWork) {
                Value* Dst = gepI8(B, W.CtTy, W.CacheBuf);
                Value* Src = gepI8(B, W.CtTy, W.CtGV);
                B.CreateMemCpy(Dst, Align(1), Src, Align(1),
                    ConstantInt::get(I64Ty, W.CtBytes));
                Value* NcPtr = gepI8(B, W.NonceTy, W.NonceGV);
                B.CreateCall(DecryptFn, {
                    Dst,
                    ConstantInt::get(I32Ty, (uint32_t)(W.CtBytes - 1)),
                    NcPtr
                    });
            }
            B.CreateRetVoid();
        ...
        for (auto& Cand : Cands) {
            ...
            if (!Ctx.ChaChaKeyGV) {
                ...
                Ctx.ChaChaKeyGV = new GlobalVariable(
                    ...
                    ".strenc.ck");
                Ctx.ChaChaKeyGV->setSection(strEncSectionName(M, "ck"));
