...
namespace {
  ...
  // Print which state caused the problem
	...
	bool FlaImpl::flattenFunction(FlaCtx& PCtx)
	{
		...
		// demoteForCFGChange returns false if its fixed-point loop didn't
		// converge within its iteration cap -- F is left valid but may still
		// have values crossing block boundaries, so it's not safe to
		// restructure its CFG below. Skip flattening for this function
		// (same graceful-skip path as the !prepareFlattening case) rather
		// than build a dispatcher on top of an incompletely-demoted function.
		if (!llvm::obf::demoteForCFGChange(F, DemotedAllocas))
			return false;
