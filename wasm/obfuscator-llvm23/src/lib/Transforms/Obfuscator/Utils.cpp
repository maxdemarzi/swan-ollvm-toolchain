...
namespace llvm::obf {
	...
	// Returns true if F is safe for the caller to proceed with a CFG-changing
	// transform (flattening) afterward -- i.e. every PHI and every
	// cross-block-escaping value has been demoted to a stack slot. Returns
	// false if the fixed-point loop below didn't converge within
	// MaxIterations: F itself is still left in valid (if only partially
	// demoted) IR -- every DemoteRegToStack/DemotePHIToStack call below is
	// unconditionally correct on its own -- but the caller must NOT proceed
	// to restructure F's CFG, since some value may still cross a block
	// boundary the flattening dispatcher would break.
	bool demoteForCFGChange(llvm::Function& F, llvm::SmallVectorImpl<llvm::AllocaInst*>& OutNewAllocas) {
		...
		// Each iteration re-scans the whole function and demotes whatever
		// still escapes its block; this is meant to reach a fixed point in a
		// small, bounded number of passes (typically 1-2: demoting a value
		// used by a PHI that's ALSO being demoted this same iteration can
		// surface one more round of newly-inserted reload instructions, but
		// no more than that for well-formed input). Some pathological CFG
		// shapes have been observed to never converge (a real hang, not just
		// slow -- confirmed via a live debugger attach on a hung `opt`
		// process, backtrace stuck in this loop). Cap it rather than spin
		// forever; the caller checks the return value and skips flattening
		// for this function instead of restructuring a CFG that still has
		// un-demoted escaping values.
		constexpr unsigned MaxDemoteIterations = 64;
		unsigned DemoteIteration = 0;
		...
		do {
			...
			if ((!TmpReg.empty() || !TmpPhi.empty()) && ++DemoteIteration >= MaxDemoteIterations) {
				if (llvm::ObfVerbose)
					llvm::errs() << "demoteForCFGChange: did not converge after "
						<< MaxDemoteIterations << " iterations on " << F.getName()
						<< " -- skipping CFG-changing transform for this function\n";
				return false;
			}
		...
		(void)Changed; // tracked for potential future diagnostics; not part of the return contract
		return true;
	}
